...
benA <- read.csv(file="benAtest.csv")
...
centroid.benA <- c(mean(scores[1:159,1]) , mean(scores[1:159,2]))
centroid.benB <- c(mean(scores[160:258,1]), mean(scores[160:258,2]))
centroid.neo <- c(mean(scores[259:434,1]), mean(scores[259:434,2]))
...
plot(scores[1:159,1], scores[1:159,2], xlab="PC1 (66%)", ylab="PC2 (19%)", pch=16, col=1, xlim=c(-.15,.15), ylim=c(-.15,.15))
points(scores[160:258,1], scores[160:258,2], pch=8, col=2)
points(scores[259:434,1], scores[259:434,2], pch=17, col=3)
