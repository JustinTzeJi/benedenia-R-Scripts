...
benA <- read.csv(file="benAhard.csv")
benB <- read.csv(file="benBhard.csv")
neo <- read.csv(file="neoBhard.csv")
...
xhist <- hist(scores[,1], breaks=seq(-.12,.15,0.01), plot=FALSE)
yhist <- hist(scores[,2], breaks=seq(-.25,.3,0.01), plot=FALSE)
...
plot(scores[1:160,1], scores[1:160,2], xlab="PC1 (92%)", ylab="PC2 (4%)", pch=18, col=1, xlim=c(-.12,.15), ylim=c(-.25,.3))
...
legend(-.12, .31, cex=0.8, pch=c(18,8,17), col = (1:4), expression(paste(italic("Benedenia A")), paste(italic("Benedenia B")), paste(italic("Neobenedenia B"))))
