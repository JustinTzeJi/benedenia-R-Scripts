...
benA <- read.csv(file="benAhardant.csv")
benB <- read.csv(file="benBhardant.csv")
neo <- read.csv(file="neoBhardant.csv")
benL <- read.csv(file="benLhardant.csv")
...
yhist <- hist(scores[,2], breaks=seq(-.2,.3,0.01), plot=FALSE)
...
plot(scores[1:160,1], scores[1:160,2], xlab="PC1 (84%)", ylab="PC2 (9%)", pch=16, col=1, xlim=c(-.15,.15), ylim=c(-.2,.3))
...
legend(-.15, .3, cex=0.8, pch=c(16,8,17,15), col = (1:4), expression(paste(italic("Benedenia A")), paste(italic("Benedenia B")), paste(italic("Neobenedenia B")), paste(italic("Benedenia lutjani")) ))
