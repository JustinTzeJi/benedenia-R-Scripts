pool <- read.csv(file="compileAlocal.csv")
...
autoplot(pca, data = pool, shape = 'SP', 
         frame = TRUE, frame.type = 't', frame.colour = 'SP', size = 2) + 
  scale_shape_manual(values=c(1,3,25)) + 
  theme_classic() + 
  theme(legend.position="bottom")


autoplot(pca, data = pool, shape = FALSE, label = FALSE,
         frame = TRUE, frame.type = 't', frame.colour = 'SP', frame.alpha = 0) + 
  scale_shape_manual(values=c(1,3,25)) + 
  theme_classic() + 
  theme(legend.position="bottom")

ggsave(file="test.svg",width = 12, height = 12, units=c("cm"), dpi = 300)
