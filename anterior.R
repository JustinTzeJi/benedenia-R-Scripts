...
benA <- read.csv(file="benAant.csv")
benB <- read.csv(file="benBant.csv")
neo <- read.csv(file="neoBant.csv")
...
plot(scores[1:160,1], scores[1:160,2], xlab="PC1 (84%)", ylab="PC2 (9%)", pch=18, col=1, xlim=c(-.15,.15), ylim=c(-.15,.15))
...
legend(-.15, -.08, cex=0.8, pch=c(18,8,17), col = (1:4), expression(paste(italic("Benedenia A")), paste(italic("Benedenia B")), paste(italic("Neobenedenia B"))))
