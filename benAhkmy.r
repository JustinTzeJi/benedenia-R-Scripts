...
hk <- read.csv(file="benAhk.csv")
my <- read.csv(file="benAmy.csv")
...
centroid.hk <- c(mean(scores[1:38,1]) , mean(scores[1:38,2]))
centroid.my <- c(mean(scores[39:160,1]), mean(scores[39:160,2]))


#remember to match this with the xlim and ylim in the plot function
xhist <- hist(scores[,1], breaks=seq(-.3,.15,0.01), plot=FALSE)
...
plot(scores[1:38,1], scores[1:38,2], xlab="PC1 (73%)", ylab="PC3 (5%)", pch=16, col=1, xlim=c(-.3,.15), ylim=c(-.2,.2))
points(scores[39:160,1], scores[39:160,2], pch=8, col=2)

#this is your legend, you can change the fonts, font size, color, position of legend box etc...
abline(h=0, lty=3) ; abline(v=0, lty=3)
legend(-.3, .2, cex=0.8, pch=c(16,8), col = (1:2), expression(paste(italic("Hong Kong")), paste(italic("Malaysia")) ))
